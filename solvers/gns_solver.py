...
def get_scheduling_and_material_use_actions(instance: Instance, graph: GraphInstance):
    ...
    for operation_id in graph.Q.operation_queue:
        ...
        if graph.operation(operation_id, 'remaining_resources')>0: # 1. Try for scheduling (and check for sync)
            for rt in graph.remaining_types_of_resources[p][o]:
        ...
        if graph.operation(operation_id, 'remaining_materials')>0: # 2. Try for material use
            for rt in graph.remaining_types_of_materials[p][o]:
...
def get_feasible_actions(instance: Instance, graph: GraphInstance):
    actions = [] if not graph.Q.item_queue else get_outourcing_actions(instance, graph)
    type = OUTSOURCING
    execution_times: list[int] = []
    if not actions:
        actions, execution_times, type = get_scheduling_and_material_use_actions(instance, graph)
...
def outsource_item(graph: GraphInstance, instance: Instance, item_id: int, enforce_time: bool=False, outsourcing_time: int=-1):
    p, e = graph.items_g2i[item_id]
    ...
    for child in graph.direct_children[p][e]:
        _, child_end_time, child_cost = outsource_item(graph, instance, graph.items_i2g[p][child], enforce_time=True, outsourcing_time=outsourcing_start_time)
        cost += child_cost
    ...
    for o in instance.loop_item_operations(p,e):
        ...
        if op_id in graph.Q.operation_queue:
            graph.Q.remove_operation(op_id)
        graph.update_operation(op_id, [
            ('remaining_resources', 0.0),
            ('remaining_materials', 0.0),
            ('remaining_time', 0.0)]) 
        for rt in graph.remaining_types_of_resources[p][o] + graph.remaining_types_of_materials[p][o]:
            for r in graph.res_by_types[rt]:
...
def apply_outsourcing_to_direct_parent(instance: Instance, graph: GraphInstance, p: int, e: int, end_date: int):
    ...
    for o in graph.first_physical_operations[p][_parent]:
        ...
        for previous in graph.previous_operations[p][o]:
        ...
        if next_good_to_go:
            ...
            graph.Q.add_operation(graph.operations_i2g[p][o])

# Apply use material to an operation
def apply_use_material(graph: GraphInstance, instance: Instance, operation_id: int, material_id: int, use_material_time: int):
    p, o              = graph.operations_g2i[operation_id]
    ...
    graph.remaining_types_of_materials[p][o].remove(rt)

# Schedule an operation on a resource
def schedule_operation(graph: GraphInstance, instance: Instance, operation_id: int, resource_id: int, scheduling_time: int):
    ...
    graph.remaining_types_of_resources[p][o].remove(rt)
...
def schedule_other_resources_if_simultaneous(instance: Instance, graph: GraphInstance, operation_id: int, resource_id: int, p: int, o: int, sync_time: int, operation_end: int):
            ...
            for r in graph.res_by_types[rt]:
                if instance.finite_capacity[r]:
                    ...
                    if scaled_res_ready_time <= sync_time:
                        ...
                        op_end, _, ex_time = schedule_operation(graph, instance, operation_id, other_resource_id, sync_time)
                        operation_end      = max(operation_end, op_end)
                        total_ex_time     += ex_time
                        break
                else:
                    found_suitable_r = True
                    apply_use_material(graph, instance, operation_id, graph.materials_i2g[r], sync_time)
                    break
...
def try_to_open_next_operations(graph: GraphInstance, instance: Instance, operation_id: int): 
    p, o = graph.operations_g2i[operation_id]
    e = graph.item_of_operations[p][o]
    op_end_time = graph.operation(operation_id, 'end_time')
    for _next in graph.next_operations[p][o]:
        next_good_to_go = True
        next_id = graph.operations_i2g[p][_next]
        for previous in graph.previous_operations[p][_next]:
            if not graph.is_operation_complete(graph.operations_i2g[p][previous]):
                next_good_to_go = False
                break
...
def solve_one_graph(instance: Instance, graph: GraphInstance, agents: Agents, train: bool, device: str, greedy: bool=False, REPLAY_MEMORY: Tree=None, episode: int=0, debug: bool=False):
    workload_removed: int = 0
    poss_actions, actions_type, execution_times = get_feasible_actions(instance, graph)
    ...
    if train:
        state_before_action: HistoricalState = HistoricalState(REPLAY_MEMORY, state, poss_actions, graph.current_cmax, graph.current_cost, graph.get_last_action())
        if REPLAY_MEMORY.init_state is None:
            REPLAY_MEMORY.init_state = state_before_action
    idx = select_next_action(agents, REPLAY_MEMORY, actions_type, state, poss_actions, graph.alpha, train, episode, greedy)
    if actions_type == OUTSOURCING: # Outsourcing action
        ...
        if outsourcing_choice == YES:
            _outsourcing_time, _end_date, _price = outsource_item(graph, instance, item_id, enforce_time=False)
            apply_outsourcing_to_direct_parent(instance, graph, p, e, _end_date)
            graph.current_cmax = max(graph.current_cmax, _end_date)
            graph.current_cost = graph.current_cost + _price
            graph.Q.remove_item(item_id)
        ...
        _operation_end, _actual_scheduling_time, workload_removed = schedule_operation(graph, instance, operation_id, resource_id, execution_times[idx])
        if instance.simultaneous[p][o]:
            DEBUG_PRINT("\t >> Simulatenous...")
            _operation_end, total_execution_time = schedule_other_resources_if_simultaneous(instance, graph, operation_id, resource_id, p, o, _actual_scheduling_time, _operation_end)
            workload_removed += total_execution_time
        if graph.is_operation_complete(operation_id):
            graph.Q.remove_operation(operation_id)
            try_to_open_next_operations(graph, instance, operation_id)
        DEBUG_PRINT(f"End of scheduling at time {_operation_end}...")
        graph.current_cmax = max(graph.current_cmax, _operation_end)
            ...
            apply_use_material(graph, instance, operation_id, material_id, execution_times[idx])
            if graph.is_operation_complete(operation_id):
                graph.Q.remove_operation(operation_id)
                try_to_open_next_operations(graph, instance, operation_id)
            graph.current_cmax = max(graph.current_cmax, execution_times[idx])
    if train:
        is_first_state: bool = graph.is_first_state()
        graph.actions.append(Action(idx, actions_type, target, value, workload_removed, state_before_action if not is_first_state else None))
...
def solve(instance: Instance, agents: Agents, train: bool, device: str, greedy: bool=False, REPLAY_MEMORY: Tree=None, episode: int=0, debug: bool=False):
    ...
    graph = translate(i=instance, device=device)
    graph.alpha = torch.tensor([instance.w_makespan], device=device)
    if train:
        REPLAY_MEMORY.init_tree(graph.alpha, graph.lb_Cmax, graph.lb_cost, graph.ub_Cmax, graph.ub_cost)
    DEBUG_PRINT(f"Init Cmax: {graph.lb_Cmax}->{graph.ub_Cmax} - Init cost: {graph.lb_cost}$ - Max cost: {graph.ub_cost}$")
    graph.init_queue(instance, graph)
    while not graph.Q.done():
        solve_one_graph(instance, graph, agents, train, device, greedy=greedy, REPLAY_MEMORY=REPLAY_MEMORY, episode=episode)
    if train:
        last_action: Action = graph.get_last_action()
        last_action.next_state = HistoricalState(REPLAY_MEMORY, graph.to_state(device=device), [], graph.current_cmax, graph.current_cost, last_action)
        REPLAY_MEMORY.add_or_update_action(graph.get_base_action(), final_makespan=graph.current_cmax, final_cost=graph.current_cost, need_rewards=True, device=device)
    else:
        return graph.current_cmax, graph.current_cost
