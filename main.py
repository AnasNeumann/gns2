...
if __name__ == '__main__':
    ...
    if to_bool(args.train):
            # python gns_solver.py --train=true --mode=prod --version=1 --interactive=true --path=./
            train(version=_version, itrs=_itrs, agents=agents, path=args.path, device=_device, debug=_debug_mode)
    else:
        if to_bool(args.target):
            # SOLVE ACTUAL INSTANCE: python gns_solver.py --target=true --size=xxl --id=151 --train=false --mode=test --path=./ --version=1 --itrs=0
            # TRY ON DEBUG INSTANCE: python gns_solver.py --target=true --size=d --id=debug --train=false --mode=test --path=./ --version=1 --itrs=0
            ...
            # python gns_solver.py --train=false --target=false --mode=prod --path=./ --version=1 --itrs=0
